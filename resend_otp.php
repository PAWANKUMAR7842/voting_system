...
if ($stmt->affected_rows > 0) {
    ...
    try {
        ...
        $mail->send();
        echo "<script>alert('New OTP sent to your email!'); window.location.href='verify_otp.php?email=$email';</script>";
    } catch (Exception $e) {
        echo "<script>alert('Failed to send OTP via email. Error: " . $mail->ErrorInfo . "'); window.location.href='verify_otp.php?email=$email';</script>";
    }
} else {
    echo "<script>alert('Failed to generate new OTP!'); window.location.href='verify_otp.php?email=$email';</script>";
}
